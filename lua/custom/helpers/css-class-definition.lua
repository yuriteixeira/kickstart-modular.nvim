...
local function is_markup_buffer(bufnr) return MARKUP_FILETYPES[vim.bo[bufnr].filetype] == true end
...
local function escape_regex(value) return (value:gsub('([\\%(%)%.%%%+%-%*%?%[%]%^%$%{%}%|])', '\\%1')) end

local function css_escape(value) return (value:gsub('([^%w_-])', '\\%1')) end
